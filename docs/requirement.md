...
## v4.1移行の現状（2026-09-08）

v4.1を採用し、新規設定の既定へ反映した。v3と同等の処理時間・メモリ使用量は求めず、旧比較値と欠測を診断として残す。正式条件・生成成功・v4の60秒期限と共存/取消の要件を維持する。品質は利用者が許容済み。実UIについても利用者の起動・一通りの動作確認と最終承認を受領した。詳細は[移行計画の受入表](plan_20260907_irodori_v4/90_status.md)。
